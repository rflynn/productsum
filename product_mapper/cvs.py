...
'''
map a document archived from cvs.com to zero or more products
'''
...
class ProductCVS(object):
    ...
    def __init__(self, id=None, url=None, merchant_name=None, slug=None,
        ...
        if self.name:
            if self.name.endswith(" - CVS.com"):
                self.name = self.name[:-len(" - CVS.com")]

        if self.title:
            if self.title.endswith(" - CVS.com"):
                self.title = self.title[:-len(" - CVS.com")]
...
class ProductsCVS(object):
    ...
    def get_custom(soup, url, og):
        ...
        # color
        tag = soup.find('dl', {'class': 'refinements'})
        #print 'tag:', tag
        if tag:
            try:
                dt = tag.find('dt', text=re.compile('^Primary Color'))
                #print 'dt:', dt
                if dt:
                    dd = dt.findNext('dd')
                    #print 'dd:', dd
                    if dd:
                        color = normstring(dd.get_text())
    ...
    def from_html(cls, url, html, updated=None):
        ...
        prodid = (nth(utag.get('product_sku_id'), 0)
                    or og.get('product:mfr_part_no')
        ...
        if prodid:
            ...
            p = ProductCVS(
                ...
                size=(nth(utag.get('product_size'), 0)
                            or custom.get('size')
                            or None),
...
def do_file(url, filepath):
    ...
    with gzip.open(filepath) as f:
        html = f.read()
    return ProductsCVS.from_html(url, html)
...
if __name__ == '__main__':
    ...
    url = 'http://www.cvs.com/shop/beauty/makeup/lips/wet-n-wild-megalast-lipcolor-908c-sugar-plum-fairy-skuid-725626'
    filepath = 'test/www.cvs.com-shop-beauty-makeup-lips-wet-n-wild-megalast-lipcolor-908c-sugar-plum-fairy-skuid-725626.gz'

    url = 'http://www.cvs.com/shop/beauty/skin-care/body/aquaphor-healing-ointment-skuid-412242'
    filepath = 'test/www.cvs.com-shop-beauty-skin-care-body-aquaphor-healing-ointment-skuid-412242.gz'
