...
'''
map a document archived from nyxcosmetics.com to zero or more products
'''
...
MERCHANT_SLUG = 'nyxcosmetics'


class ProductNyxCosmetics(object):
    VERSION = 0
    def __init__(self, id=None, url=None, merchant_name=None, slug=None,
        ...
        if self.name:
            if self.name.endswith(" | NYX Cosmetics"):
                self.name = self.name[:-len(" | NYX Cosmetics")]

        if self.title:
            if self.title.endswith(" | NYX Cosmetics"):
                self.title = self.title[:-len(" | NYX Cosmetics")]
...
class ProductsNyxCosmetics(object):
    ...
    def get_custom(soup, url, og):
        ...
        try:
            br = soup.find('span', itemprop='breadcrumb')
            if br:
                breadcrumbs = [x for x in
                                [normstring(b)
                                    for b in re.split('\W+:\W+', normstring(br.get_text()))]
                                        if x] or None
        except:
            traceback.print_exc()
        ...
        try:
            sc = soup.find('script', text=lambda t: t and 'app.page.setContext({' in t)
            if sc:
                m = re.search('({.*})', sc.text)
                if m:
                    objtxt = m.groups(0)[0]
                    ...
                    if 'trackerData' in obj:
                        td = obj['trackerData']
                        try:
                            name = td.get('productName') or None
                            sku = td.get('productSku') or None
                            price = td.get('unitPrice') or None
                            if price is not None:
                                price = str(price)
                            if 'productStatus' in td:
                                # e.g. '1 Item(s) In Stock'
                                # NOTE: this is not known to be accurate...
                                in_stock = 'In Stock' in td['productStatus']
                            category = td.get('productCategory') or None
                            if 'categoryBreakout' in td:
                                breadcrumbs = [normstring(x)
                                                for x in re.split('\s+>\s+',
                                                                    td['categoryBreakout'])] or None
                        except:
                            traceback.print_exc()
                    if 'eeProductsOnPage' in obj:
                        try:
                            upcs = sorted(obj['eeProductsOnPage'].keys())
                            if upcs:
                                upc = upcs[0]
                        except:
                            pass
...
def do_file(url, filepath):
    ...
    with gzip.open(filepath) as f:
        html = f.read()
    return ProductsNyxCosmetics.from_html(url, html)
...
if __name__ == '__main__':
    ...
    url = 'http://www.nyxcosmetics.com/soft-matte-lip-cream/NYX_007.html'
    filepath = 'test/www.nyxcosmetics.com-soft-matte-lip-cream-NYX_007.html.gz'
