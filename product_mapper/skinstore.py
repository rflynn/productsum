...
'''
map a document archived from skinstore.com to zero or more products
'''
...
MERCHANT_SLUG = 'skinstore'


class ProductSkinstore(object):
    VERSION = 0
    def __init__(self, id=None, url=None, merchant_name=None, slug=None,
        ...
        if self.name:
            if self.name.endswith(" | Skinstore"):
                self.name = self.name[:-len(" | Skinstore")]

        if self.title:
            if self.title.endswith(" | Skinstore"):
                self.title = self.title[:-len(" | Skinstore")]
...
class ProductsSkinstore(object):
    ...
    def get_custom(soup, url, og):
        ...
        try:
            u = URL(url_canonical)
            if u.path and u.path.startswith('/p-'):
                m = re.search('^/p-(\d{3,8})-', u.path)
                if m:
                    sku = m.groups(0)[0]
        except:
            traceback.print_exc()
        ...
        try:
            de = soup.find('div', id='aDescriptionBody')
            #print 'de:', de
            if de:
                descr = normstring(' '.join(p.get_text() for p in de.findAll('p')))
        except:
            traceback.print_exc()
...
def do_file(url, filepath):
    ...
    with gzip.open(filepath) as f:
        html = f.read()
    return ProductsSkinstore.from_html(url, html)
...
if __name__ == '__main__':
    ...
    url = 'http://www.skinstore.com/p-3496-too-faced-lip-injection-extreme.aspx'
    filepath = 'test/www.skinstore.com-p-3496-too-faced-lip-injection-extreme.aspx.gz'
