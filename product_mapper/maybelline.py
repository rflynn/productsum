...
'''
map a document archived from maybelline.com to zero or more products
'''
...
MERCHANT_SLUG = 'maybelline'


class ProductMaybelline(object):
    VERSION = 0
...
class ProductsMaybelline(object):
...
<script type="text/javascript">
    ...
    GTMTagging.GTMVar.productCategory = 'Lip Color';
    GTMTagging.GTMVar.categoryBreakout = ' > Lip Makeup > Lip Color > ';
    GTMTagging.GTMVar.productName = 'Color Sensational® Rebel Bloom';
    GTMTagging.GTMVar.productSku = '';
    GTMTagging.GTMVar.productPrice = '7.49';
    GTMTagging.GTMVar.ecommerceStatus = 'Viewer';

    GTMTagging.GTMVar.addToBagType = 'Product page';

    GTMTagging.pageViewTag();

    GTMTagging.GTMVar.searchKeyword = '';
    GTMTagging.GTMVar.searchCategory = '';
</script>
        '''
        tag = soup.find('script', text=lambda t: t and 'GTMTagging.GTMVar.productPrice =' in t)
        if tag:
            x = {}
            for line in tag.text.split('\n'):
                if line:
                    m = re.search(r"GTMTagging\.GTMVar\.(\S+) = '([^']*)';", line)
                    if m:
                        x[m.groups(0)[0]] = m.groups(0)[1]
            #pprint(x)
            if x:
                category = category or x.get('productCategory') or None
                breadcrumbs = category or x.get('categoryBreakout') or None
                price = price or x.get('productPrice') or None
                if x.get('categoryBreakout'):
                    try:
                        breadcrumbs = x['categoryBreakout'].strip(' >').split(' > ')
                    except Exception as e:
                        print e

        '''
        <script type="text/javascript">
            Site.productdetail.colorJson = [
        '''
        tag = soup.find('script', text=lambda t: t and 'Site.productdetail.colorJson = ' in t)
        if tag:
            ...
            if m:
                try:
                    ...
                    #pprint(obj)
                    colors = [x for x in
                                [normstring(o.get('name'))
                                    for o in obj] if x] or None
                except Exception as e:
                    print e
    ...
    def from_html(cls, url, html, updated=None):
        ...
        prodid = (og.get('product:mfr_part_no')
                    ...
                    or custom.get('sku')
                    or nth(sp.get('productId'), 0) # this one is expected for maybelline.com
                    or None)
...
def do_file(url, filepath):
    ...
    with gzip.open(filepath) as f:
        html = f.read()
    return ProductsMaybelline.from_html(url, html)
...
if __name__ == '__main__':
    ...
    url = 'http://www.maybelline.com/Products/Lip-Makeup/Lip-Color/Color-Sensational-Rebel-Bloom.aspx'
    filepath = 'test/www.maybelline.com-Products-Lip-Makeup-Lip-Color-Color-Sensational-Rebel-Bloom.aspx.gz'

    url = 'http://www.maybelline.com/Products/face-makeup/Bronzer/fit-me-bronzer.aspx'
    filepath = 'test/www.maybelline.com-Products-face-makeup-Bronzer-fit-me-bronzer.aspx.gz'
