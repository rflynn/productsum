...
'''
map a document archived from toryburch.com to zero or more products
'''
...
MERCHANT_SLUG = 'toryburch'


class ProductToryBurch(object):
    VERSION = 0
...
class ProductsToryBurch(object):
    ...
    def get_custom(soup, url, og):
        ...
        jQuery(document).ready(function(){
                app.ProductCache = new app.Product({ data:
{
"source": "null",
"name": "TEXTURED BURLAP DRESS",
"brand": "Tory Burch",
        '''
        sc = soup.find('script', text=lambda t: t and 'app.ProductCache = new app.Product({' in t)
        #print 'sc:', sc
        if sc:
            try:
                ...
                if m:
                    ...
                    o = obj['data']
                    #pprint(o)
                    sku = o.get('ID') or None
                    brand = o.get('brand') or None
                    name = o.get('name') or None
                    in_stock = o.get('inStock')
                    url_canonical = url_canonical or o.get('url') or None
                    v = o.get('variations')
                    #print 'v:', v
                    if v:
                        ...
                        #print 'attr:', attr
                        if attr:
                            attr = {v['id']: v for v in attr}
                            #print 'attr:'
                            #pprint(attr)
                            c = attr.get('color')
                            colors = [o.get('val') for o in c['vals']]
                            s = attr.get('size')
                            sizes = [o.get('val') for o in s['vals']]
            except:
                traceback.print_exc()
    ...
    def from_html(cls, url, html, updated=None):
        ...
        prodid = (og.get('product:mfr_part_no')
                    ...
                    or nth(sp.get('sku'), 0) # should exist in toryburch
                    or custom.get('sku') # should exist in toryburch
                    or None)
...
def do_file(url, filepath):
    ...
    with gzip.open(filepath) as f:
        html = f.read()
    return ProductsToryBurch.from_html(url, html)
...
if __name__ == '__main__':
    ...
    url = 'http://www.toryburch.com/textured-burlap-dress/51151453.html?cgid=clothing-dresses&dwvar_51151453_color=422&start=1'
    filepath = 'test/www.toryburch.com-textured-burlap-dress-51151453.html.gz'

    url = 'http://www.toryburch.com/britten-satchel/31159879.html?dwvar_31159879_size=OS&cgid=handbags-britten&start=1&dwvar_31159879_color=001'
    filepath = 'test/www.toryburch.com-britten-satchel-31159879.html.gz'
