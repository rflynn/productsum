...
'''
map a document archived from selfridges.com to zero or more products
'''
...
MERCHANT_SLUG = 'selfridges'


class ProductSelfridges(object):
    VERSION = 0
    def __init__(self, id=None, url=None, merchant_name=None, slug=None,
        ...
        if self.name:
            if self.name.endswith(" | Selfridges.com"):
                self.name = self.name[:-len(" | Selfridges.com")]

        if self.title:
            if self.title.endswith(" | Selfridges.com"):
                self.title = self.title[:-len(" | Selfridges.com")]
...
class ProductsSelfridges(object):
    ...
    def get_custom(soup, url, og):
        ...
        fo = soup.find('form', {'name': 'OrderItemAddForm'})
        if fo:
            try:
                inp = {i['name']: i['value']
                            for i in fo.findAll('input',
                                        {'type':'hidden', 'name':True, 'value':True})}
        ...
        if dko:
            ...
            if opt:
                try:
                    sizes = [x for x in
                                [normstring(o.get('value'))
                                    for o in opt] if x] or None
                except:
                    traceback.print_exc()
    ...
    def from_html(cls, url, html, updated=None):
        ...
        prodid = (og.get('product:mfr_part_no')
                    ...
                    or nth(sp.get('productID'), 0) # this one is expected for selfridges.com
                    or None)
...
def do_file(url, filepath):
    ...
    with gzip.open(filepath) as f:
        html = f.read()
    return ProductsSelfridges.from_html(url, html)
...
if __name__ == '__main__':
    ...
    url = 'http://www.selfridges.com/US/en/cat/osim-ugalaxy-eye-massager_486-3001191-R112/'
    filepath = 'test/www.selfridges.com-US-en-cat-osim-ugalaxy-eye-massager_486-3001191-R112.gz'

    url = 'http://www.selfridges.com/US/en/cat/jimmy-choo-lorelai-100-leather-heeled-courts_834-10132-LORELAI100F/?previewAttribute=Nude%2Fchampagne+mix'
    filepath = 'test/www.selfridges.com-US-en-cat-jimmy-choo-lorelai-100-leather-heeled-courts_834-10132-LORELAI100F.gz'
