...
'''
map a document archived from therealreal.com to zero or more products
'''
...
MERCHANT_SLUG = 'therealreal'


class ProductTheRealReal(object):
    VERSION = 0
    def __init__(self, id=None, url=None, merchant_name=None, slug=None,
        ...
        if self.name:
            if self.name.endswith(" - therealreal.com"):
                self.name = self.name[:-len(" - therealreal.com")]
            self.name = self.name or None

        if self.title:
            if self.title.endswith(" - therealreal.com"):
                self.title = self.title[:-len(" - therealreal.com")]

        if self.upc:
            self.upc = str(self.upc)

    def __repr__(self):
        return ('''ProductTheRealReal:
    id............... %s
    url.............. %s
    merchant_name.... %s
    merchant_sku..... %s
...
class ProductsTheRealReal(object):
    ...
    def get_custom(soup, url, og):
        ...
        br = soup.find('ul', {'class': 'breadcrumb-list'})
        if br:
            try:
                breadcrumbs = [normstring(a.get_text())
                                    for a in br.findAll('li')] or None
            except:
                traceback.print_exc()

        pt = soup.find('div', id='product-thumbnails')
        if pt:
            try:
                img_urls = [x for x in
                                [i.get('src') or None
                                    for i in pt.findAll('img', src=True)]
                                        if x] or None
            except:
                traceback.print_exc()

        av = soup.find(itemprop='availability', content=True)
        if av:
            try:
                in_stock = av.get('content') == 'http://schema.org/InStock'
            except:
                traceback.print_exc()
    ...
    def from_html(cls, url, html, updated=None):
        ...
        metaprop = {tag['property']: tag['content']
                        for tag in soup.findAll('meta', {'property': True, 'content': True})}
        ...
        prodid = (og.get('product:mfr_part_no')
                    ...
                    or nth(sp.get('productID'), 0)
                    or custom.get('sku')
                    or metaprop.get('sku') # expected for therealreal
                    or None)
...
def do_file(url, filepath):
    ...
    with gzip.open(filepath) as f:
        html = f.read()
    return ProductsTheRealReal.from_html(url, html)
...
if __name__ == '__main__':
    ...
    url = 'https://www.therealreal.com/products/women/shoes/pumps/christian-louboutin-pigalle-pumps-75'
    filepath = 'test/www.therealreal.com-products-women-shoes-pumps-christian-louboutin-pigalle-pumps-75.gz'
